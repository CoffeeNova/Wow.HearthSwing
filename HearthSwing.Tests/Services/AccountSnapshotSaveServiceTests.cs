...
public class AccountSnapshotSaveServiceTests
{
    ...
    public void Save_WhenAccountIsNew_CopiesWholeAccountAndUpdatesCatalog()
    {
        ...
        _catalog
            .GetById(savedAccount.Id)
            .Returns(savedAccount with { LastSavedUtc = DateTimeOffset.UtcNow });

        _fileSystem.DirectoryExists(savedAccount.SnapshotPath).Returns(false);
        _fileSystem
            .GetFiles(liveAccount.FolderPath, "*", SearchOption.TopDirectoryOnly)
            .Returns([@"C:\Game\WTF\Account\Alpha\bindings-cache.wtf"]);
        _fileSystem
            .GetDirectories(liveAccount.FolderPath)
            .Returns([
        ...
        _fileSystem
            .GetFiles(@"C:\Game\WTF\Account\Alpha\Firemaw", "*", SearchOption.TopDirectoryOnly)
            .Returns([]);
        _fileSystem
            .GetDirectories(@"C:\Game\WTF\Account\Alpha\Firemaw")
            .Returns([@"C:\Game\WTF\Account\Alpha\Firemaw\Hero"]);
        _fileSystem
            .GetFiles(@"C:\Game\WTF\Account\Alpha\Firemaw\Hero", "*", SearchOption.TopDirectoryOnly)
    ...
    public void Save_WhenExistingAccountAndCharacterSelected_ReplacesOnlySelectedCharacter()
    {
        ...
        var savePlan = new AccountSavePlan
        {
            ...
            SelectedCharacters =
            [
                new CharacterSaveSelection { RealmName = "Firemaw", CharacterName = "Hero" },
            ],
        };
        ...
        _fileSystem
            .Received()
            .DeleteDirectory(@"C:\Profiles\alpha\Account\Alpha\Firemaw\Hero", true);
        _fileSystem
            .Received()
            .CopyFile(
                @"C:\Game\WTF\Account\Alpha\Firemaw\Hero\layout-local.txt",
        ...
        _fileSystem
            .DidNotReceive()
            .DeleteDirectory(@"C:\Profiles\alpha\Account\Alpha\SavedVariables", true);
    }

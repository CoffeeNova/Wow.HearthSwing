...
public class AccountSwitchServiceTests
{
    ...
    public void SetUp()
    {
        ...
        _settings.Current.Returns(new AppSettings { GamePath = @"C:\Game", ProfilesPath = @"C:\Profiles" });
    ...
    public void SwitchTo_WhenSnapshotExists_CopiesSnapshotToLiveAccountAndMarksActive()
    {
        ...
        _fileSystem.GetFiles(savedAccount.SnapshotPath, "*", SearchOption.TopDirectoryOnly)
            .Returns([@"C:\Profiles\alpha\Account\Alpha\bindings-cache.wtf"]);
        _fileSystem.GetDirectories(savedAccount.SnapshotPath)
            .Returns([@"C:\Profiles\alpha\Account\Alpha\Firemaw"]);
        _fileSystem.GetFiles(@"C:\Profiles\alpha\Account\Alpha\Firemaw", "*", SearchOption.TopDirectoryOnly)
            .Returns([]);
        _fileSystem.GetDirectories(@"C:\Profiles\alpha\Account\Alpha\Firemaw")
        ...
        _catalog.Received().SetActiveAccount(
            Arg.Is<ActiveAccountState>(state => state.SavedAccountId == "alpha" && state.AccountName == "Alpha")
        );
    }
    ...
    public void RestoreActiveAccount_WhenActiveAccountExists_ReappliesSavedSnapshot()
    {
        // Arrange
        var savedAccount = BuildSavedAccount();
        _catalog.GetActiveAccount().Returns(new ActiveAccountState { SavedAccountId = "alpha", AccountName = "Alpha" });
        _catalog.GetById("alpha").Returns(savedAccount);
        _fileSystem.DirectoryExists(savedAccount.SnapshotPath).Returns(true);
        _fileSystem.GetFiles(savedAccount.SnapshotPath, "*", SearchOption.TopDirectoryOnly).Returns([]);

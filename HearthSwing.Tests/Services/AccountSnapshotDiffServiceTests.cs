...
public class AccountSnapshotDiffServiceTests
{
    ...
    private void ConfigureCommonLiveAccountFiles()
    {
        ...
        _fileSystem
            .GetFiles(liveAccountPath, "*", SearchOption.TopDirectoryOnly)
            .Returns([liveAccountRootFile]);
        _fileSystem
            .GetFiles(liveSavedVariablesPath, "*", SearchOption.AllDirectories)
            .Returns([liveSavedVariableFile]);
        _fileSystem
            .GetFiles(liveCharacterPath, "*", SearchOption.AllDirectories)
    ...
    )
    {
        ...
        const string savedSavedVariableFile =
            @"C:\Profiles\alpha\Account\Alpha\SavedVariables\Addon.lua";
        const string savedCharacterFile =
        ...
        _fileSystem
            .GetFiles(savedAccountPath, "*", SearchOption.TopDirectoryOnly)
            .Returns([savedAccountRootFile]);
        _fileSystem
            .GetFiles(savedSavedVariablesPath, "*", SearchOption.AllDirectories)
            .Returns([savedSavedVariableFile]);
        _fileSystem
            .GetFiles(savedCharacterPath, "*", SearchOption.AllDirectories)

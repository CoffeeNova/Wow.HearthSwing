...
public partial class MainViewModel : ObservableObject
{
    ...
    private void RefreshState()
    {
        ...
        CurrentAccountName = activeSavedAccount?.AccountName ?? activeSavedAccountState?.AccountName ?? "None";
        CurrentSavedAccountId = activeSavedAccount?.Id ?? activeSavedAccountState?.SavedAccountId ?? string.Empty;
    ...
    private async Task HandleSaveOnExitAsync()
    {
        ...
        {
            AppendLog($"No changes detected for account '{liveAccount.AccountName}' — skipping save.");
            return;
        }
    ...
    private async Task ConfirmSaveSelectionAsync()
    {
        ...
        try
        {
            ...
            RefreshState();
            StatusText = $"Account '{savedAccount?.AccountName ?? _pendingLiveAccount.AccountName}' saved.";
        }
        catch (Exception ex)
        {
            AppendLog($"ERROR: {ex.Message}");
    ...
    private void UpdateLiveAccounts(WowInstallation installation)
    {
        ...
        DetectedLiveAccountsSummary = installation.Accounts.Count switch
        {
            ...
            _ => $"Live accounts detected: {string.Join(", ", installation.Accounts.Select(account => account.AccountName))}",
        };
    }
    ...
    private void OpenSaveSelection(string? preselectedAccountName = null, string? title = null)
    {
        ...
        var targetAccountName = !string.IsNullOrWhiteSpace(preselectedAccountName)
            ? preselectedAccountName
            : _installation.Accounts.Count == 1
                ? _installation.Accounts[0].AccountName
                : null;
    ...
    private void BuildSaveSelectionForSelectedAccount()
    {
        ...
        SaveAccountSettingsSelected = diff.IsNewAccount || diff.AccountSettingsStatus != AccountSnapshotDiffStatus.Unchanged;

        if (diff.IsNewAccount)
        {
            SaveSelectionMessage = $"Account '{liveAccount.AccountName}' has not been saved yet. Confirm to save the entire account snapshot.";
            OnPropertyChanged(nameof(CanConfirmSaveSelection));
            return;
        }

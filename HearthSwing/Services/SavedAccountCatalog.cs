...
public sealed class SavedAccountCatalog : ISavedAccountCatalog
{
    ...
    public ActiveAccountState? GetActiveAccount()
    {
        ...
        try
        {
            ...
            {
                _logger.LogWarning(
                    "Active saved-account marker at {MarkerPath} is invalid.",
                    markerPath
                );
                return null;
            }
        ...
        catch (JsonException ex)
        {
            _logger.LogWarning(
                ex,
                "Failed to parse active saved-account marker at {MarkerPath}.",
                markerPath
            );
            return null;
        }
        catch (IOException ex)
        {
            _logger.LogWarning(
                ex,
                "Failed to read active saved-account marker at {MarkerPath}.",
                markerPath
            );
            return null;
        }
        catch (UnauthorizedAccessException ex)
        {
            _logger.LogWarning(
                ex,
                "Access denied reading active saved-account marker at {MarkerPath}.",
                markerPath
            );
            return null;
        }
    ...
    private static string GetMetadataPath(string rootPath) =>
        Path.Combine(rootPath, MetadataFileName);

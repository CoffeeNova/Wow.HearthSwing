...
public sealed class AccountSnapshotSaveService : IAccountSnapshotSaveService
{
    ...
    public SavedAccountSummary Save(WowAccount liveAccount, AccountSavePlan savePlan)
    {
        ...
        if (existingSavedAccount is null || !_fileSystem.DirectoryExists(existingSavedAccount.SnapshotPath))
        {
        ...
        else
        {
            ...
            if (savePlan.SaveAccountSettings)
                ReplaceAccountSettingsWithRollback(liveAccount.FolderPath, savedAccount.SnapshotPath);
    ...
    private void DeleteAccountSettings(string accountPath)
    {
        ...
        foreach (var filePath in _fileSystem.GetFiles(accountPath, "*", SearchOption.TopDirectoryOnly))
        {
    ...
    private void CopyAccountSettings(string sourceAccountPath, string destinationAccountPath)
    {
        ...
        foreach (
            var filePath in _fileSystem.GetFiles(sourceAccountPath, "*", SearchOption.TopDirectoryOnly)
        )
        {
    ...
    private void ReplaceDirectoryWithRollback(string source, string destination, string operation)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (!rollbackRequired || string.IsNullOrEmpty(rollbackPath) || !_fileSystem.DirectoryExists(rollbackPath))
                throw;
    ...
    private void CopyDirectory(string source, string destination)
    {
        ...
        {
            CopyDirectory(childDirectory, Path.Combine(destination, Path.GetFileName(childDirectory)));
        }
    }

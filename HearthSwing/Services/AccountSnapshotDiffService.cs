...
public sealed class AccountSnapshotDiffService : IAccountSnapshotDiffService
{
    ...
    public AccountSnapshotDiff BuildDiff(WowAccount liveAccount, SavedAccountSummary? savedAccount)
    {
        ...
        if (
            ...
            && !savedAccount.AccountName.Equals(
        ...
        var accountSettingsStatus = BuildAccountSettingsStatus(
            liveAccount,
            savedAccount,
            hasSavedSnapshot
        );
        var realms = liveAccount
            .Realms.Select(realm => BuildRealmDiff(realm, savedAccount, hasSavedSnapshot))
            .ToList();
    ...
    )
    {
        ...
        var savedRelativePaths = _layout.CollectAccountSettingsRelativePaths(
            savedAccount!.SnapshotPath
        );
    ...
    )
    {
        ...
        foreach (var relativePath in liveSet.Order())
        {
            ...
            if (
                !_fileSystem
                    .ReadAllBytes(livePath)
                    .SequenceEqual(_fileSystem.ReadAllBytes(savedPath))
            )
                return AccountSnapshotDiffStatus.Modified;
        }
    ...
    )
    {
        ...
        if (
            ...
            || materialized.All(status => status == AccountSnapshotDiffStatus.Unchanged)
        )
            return AccountSnapshotDiffStatus.Unchanged;
